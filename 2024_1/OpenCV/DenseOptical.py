...
def display_flow(img, flow, stride=40):
    ...
    norm_opt_flow = cv2.normalize(norm_opt_flow, None,
                                  0, 1, cv2.NORM_MINMAX)
...
while True:
    ...
    if init_flow:
        ...
        opt_flow = cv2.calcOpticalFlowFarneback(prev_frame, gray, None,
                                                0.5, 5, 13, 10,
                                                5, 1.1, cv2.OPTFLOW_USE_INITIAL_FLOW)

    prev_flow = np.copy(gray)

    if display_flow(opt_flow, opt_flow):
        break
...
while True:
    ...
    if not status_cap:
        break
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    if not flow_DualTVL1.getUseIni